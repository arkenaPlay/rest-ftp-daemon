...
module RestFtpDaemon
  class ActorReelAPI < Reel::Server::HTTP
    ...
    PROTO_RACK_ENV = {
      ...
      SERVER_SOFTWARE   => "rest-ftp-daemon/0.0.1".freeze,
    ...
    def initialize options = {}
      ...
      #info "RestFtpDaemon::ActorAPI.initialize: REST Gateway listening on #{@host}:#{@port}"
      super(@host, @port, &method(:on_connect))
      #@app = RestFtpDaemon::API::Test.new
      @app = RestFtpDaemon::API::Root.new
    ...
    def main
      info  "RestFtpDaemon::ActorWorker.main"

      loop do
        info "RestFtpDaemon::ActorWorker.main: tick"
        sleep 1
      end
    end
